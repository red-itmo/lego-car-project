...
function [descr, _length, robot_poses] = plannerEES(start_pose, goal_pose, v_des, _mode)
    ...
    if argn(2) == 4 & _mode == 'r' then
        descr = list();
        descr($+1) = list("StraightLine", [0, 0, 0], pose(3, :), s_end(3), v_des);
        for i = 1:2
        ...
        descr($+1) = list("StraightLine", pose(3, :), [0, 0, 0], s_end(3), v_des);
    end
    ...
    legend("Клотоида", "Клотоида", "Клотоида", "Клотоида", "Прямая");
endfunction
